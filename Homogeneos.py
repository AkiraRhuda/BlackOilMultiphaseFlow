...
def Calculate_HomogeneousFlowProperties(
    ...
    Bsw_value = Vw_sc/ (Vo_sc + Vw_sc)

    GOR_value = Vg_sc / Vo_sc

    if Bsw_value == 0:
        Vg_value = (GOR_value - Rs) * Vo_sc * Bg
    else:
        Vg_value = (Vg_sc - Vo_sc * Rs - Vw_sc * Rsw) * Bg

    Vl_value = Vo_sc * Bo + Vw_sc * Bw

    A_p_value = np.pi * D_h ** 2 / 4
    ...
    rho_L_value = rho_o * Vo_sc * Bo / Vl_value + rho_w * Vw_sc * Bw / Vl_value
