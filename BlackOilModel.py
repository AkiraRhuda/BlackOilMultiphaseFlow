...
class OilPhase:
    ...
    # Oil density #
    def ρ_ob(self):
        self.ρob = (62.4 * self.do + 0.0136 * self.RGO * self.dg) / self.Bo
...
class WaterPhase:
    ...
    def compressisoterm(self):
        ...
        if 200 <= self.T <= 270 and 1000 <= self.P <= 20000:
            S = unitsconverter.Density([self.S, unitsconverter.Density(self.ρw, 'lbm/ft3', 'd')], '%', 'mg/L')
            Cw = 1/(7.033*self.P + 0.5415*S - 537.0*self.T + 403300) # psia-1
        else:
            S = unitsconverter.Density([self.S,unitsconverter.Density(self.ρw,'lbm/ft3','d')],'%', 'mg/L')
            Cw = 1 / (7.033 * self.P + 0.5415 * S - 537.0 * self.T + 403300)  # psia-1
            Warning('Parâmetro(s) da correlação para compressibilidade da água está(ão) fora do(s) limite(s)!')
        return Cw
    ...
    def output(self):
        ...
        return self.ρw, self.Cw, self.μw