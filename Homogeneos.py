...
def Calculate_HomogeneousFlowProperties(
    ...
    if Bsw_value == 0:
        ...
        Vg_value = (Vg_sc.GetValue("m3/s") - Vo_sc.GetValue("m3/s") * Rs.GetValue("sm3/sm3") - Vw_sc.GetValue(
            "m3/s") * Rsw.GetValue("sm3/sm3")) * Bg.GetValue("m3/sm3")
    ...
    rho_L_value = rho_o.GetValue("kg/m3") * (
                Vo_sc.GetValue("m3/s") * Bo.GetValue("bbl/stb")) / Vl_value + rho_w.GetValue("kg/m3") * (
                              Vw_sc.GetValue("m3/s") * Bw.GetValue("bbl/stb")) / Vl_value

    rho_m_value = lambda_L_value * rho_L_value + lambda_G_value * rho_G.GetValue("kg/m3")

    mu_L_value = mu_o.GetValue("Pa.s") * (Vo_sc.GetValue("m3/s") * Bo.GetValue("bbl/stb")) / Vl_value + mu_w.GetValue(
        "Pa.s") * (Vw_sc.GetValue("m3/s") * Bw.GetValue("bbl/stb")) / Vl_value
...
def Calculate_dpdl_t(rho_m, f_TP, m_dot_m, x, v_m, D_h, theta, A_p, M, T, rho_G):
    ...
    aceleration_cte_value = -((m_dot_m.GetValue("kg/s") / A_p.GetValue("m2")) ** 2) * x.GetValue() * M.GetValue(
        "kg/mol") / (R.GetValue("J/mol.K") * T.GetValue("K") * rho_G.GetValue("kg/m3") ** 2)
