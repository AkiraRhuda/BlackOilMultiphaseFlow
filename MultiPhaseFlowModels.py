...
class MultiPhaseFlowModel:
    ...
    ---------
    model : str
        Informa o tipo de modelo a ser utilizado para calcular as propriedades do fluido - suporta Homogêneo .........
    temperature : float
    ...
    def __init__(self, model : str, temperature, pressure, diameters : list, rugosity, incli : list, rho_l = None, rho_o = None, rho_g = None, rho_w = None,
            do = None, dg=None, salinity = None,QLsc = None, QOsc = None, QWsc = None, QL = None, QO = None, QW = None, Bo = None, Bw = None, Bg = None,
        ...
        if (self.Bo is None and self.Bw is None or self.Bg is None or self.RGL is None  and self.RGO is None or self.Rs is None  and self.Rsw
                ...
                self.sig_wg is None and self.sig_lg is None):
            if self.do is None:
        ...
        if self.Bw is not None and self.RGL is None or self.RGL is not None and self.Bw is None:
            raise Exception('Verifique se está sendo fornecido RGL e Bw!')

        if self.RGL is not None and self.RGO is not None:
    ...
    def update_PVT_properties(self, T, P, do, dg, RGL, BSW, salinidade):
        ...
        self.flow()
        self.fractions()
        self.liquidproperties(force_update=True)


    def run(self):
        self.flow()
        self.fractions()
        self.liquidproperties()
        if self.model.lower() == 'homogeneo':
            self.model_instance = HomogeneousFlowModel(self)

            if self.length is None:
                # self.model_instance.run(incli=self.incli[0])
                self.model_instance.output(incli=self.incli[0] if isinstance(self.incli, list) else self.incli)
            else:
                ...
                self.PostProcess(self.Pressure, self.Pos)
    ...
    def liquidproperties(self, force_update=False):
    ...
    def PostProcess(self, pressure, position):
        if self.postprocess is True:
            import PostProcess
            PostProcess.run(pressure, position)
...
class HomogeneousFlowModel:
    ...
    def calculate_reynolds(self):
        self.Re = self.initial_properties.rho_m_ns * self.vm * self.initial_properties.Dh/self.initial_properties.mu_m_ns
    ...
    def pressure_drop_friction(self):
        self.dPDFdl = self.f * self.initial_properties.rho_m_ns * self.vm ** 2 / (2 * self.initial_properties.Dh)
    ...
    def pressure_drop_acceleration_coefficent(self):
        self.mass_flow = self.initial_properties.rho_m_ns * self.vm * self.initial_properties.Ap
